...
`server-log` には編集・削除された会話内容や画像URLも流れます。閲覧権限の事故が起きやすいため、`server-log` は運営だけが見られる権限にしてください。チャンネル通知を止めたい場合は `/log_toggle` で通知投稿をOFFにできます。
...
3. Renderの環境変数 `SYNC_GUILD_ID` に新しいサーバーIDを追加します。例: `1494132867909812266,新しいサーバーID`
4. Renderで再デプロイします。
5. 新しいサーバーで `/cache_setup` を実行します。
6. 運営メンバーへ `Cacheスタッフ` ロールを付けます。

...
```text
https://discord.com/oauth2/authorize?client_id=YOUR_CLIENT_ID&permissions=1101927672982&integration_type=0&scope=bot+applications.commands
```
...
`server-log` には編集・削除、リアクション、VCログ、メンバー参加/退出、ロール更新、テキストチャンネル更新が流れます。通常メッセージの送信ログは流さず、編集・削除時のBefore確認用としてDBにだけ保存します。運営メンバー以外に見えないようにしてください。
ログ投稿は通知抑制付きで送信されるため、ログが残っても通常の通知は出にくい設定です。

サーバー管理者は `/log-setting` でログ設定パネルを開き、ONにするログを選択して「確定」で反映できます。設定はサーバーIDごとに保存されるため、別サーバーには影響しません。従来の `/log_toggle` でも削除ログだけOFF、VC入室だけON、ロール更新だけOFFのような細かい切り替えができます。
